...
export const myOrder = async (data, loading) => {
  loading(true);
  try {
    const response = await axios.get(`${Baseurl}api/v1/order/allOrders`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("Token")}`,
      },
    });
...
export const subCategory_Product = async (subCategoryId , limit , Response , setLoading) => {
  setLoading(true)
  try{
    const res = await axios.get(`${Baseurl}api/v1/user/Product/list?subcategoryId=64e072197edbfdb7120b2443&page=1&limit=4`)
...
// export const getBanners = async (query , response , loading ) =>{
//   try{
//     const res = await axios.get(`${Baseurl}api/v1/Banner/getBanner?type=${query}`)
//     response()
//   }catch{}
// }
