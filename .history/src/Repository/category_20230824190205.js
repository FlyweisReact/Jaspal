...
export const indivisualProduct = async (
  ...
  try {
    ...
    ImageArr(response.data.data?.productImage);
    setLoading(false);
  } catch {
    setLoading(false);
  }
...
export const getProductVarients = async (setResponse, payload , productImage , image) => {
  try {
    const response = await axios.get(
    ...
    console.log(response.data.data[0].productImages)
