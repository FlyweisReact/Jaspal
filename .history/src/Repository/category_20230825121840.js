...
export const getProductbyCategory = async (
  ...
  try {
    const response = await axios.get(
      `${Baseurl}api/v1/user/Product/list?limit=&categoryId=${payload}`
    );
    setResponse(response.data.data.docs);
    setLoading(false);
  } catch {
...
export const getProductVarients = async (setResponse, payload , productImage , image , color , sizeArr , size) => {
  try {
    const response = await axios.get(
