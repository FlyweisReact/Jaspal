...
export const subCategory_Product = async (subCategoryId , limit , setResponse , setLoading) => {
  setLoading(true)
  try{
    const res = await axios.get(`${Baseurl}api/v1/user/Product/list?subcategoryId=${subCategoryId}&page=1&limit=${limit}`)
...
export const get_Banner = async (loading , type , response) => {
  loading(true)
  try{
    const res = await axios.get(`${Baseurl}api/v1/Banner/getBanner?type=${type}`)
...
export const filter_product = async (loading , query ,response, limi ) => {
  loading(true)
  try{
    const res = await axios.get(`${Baseurl}api/v1/user/Product/list?search=${query}`)
    const data =res.data.data.docs
    response(data)
    loading(false)
  }catch{
